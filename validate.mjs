...
const required=['index.html','app.js','manifest.webmanifest','sw.js','data/inventory.json','assets/icon.svg'];
const errors=[];
for(const f of required)if(!fs.existsSync(path.join(root,f)))errors.push('Falta '+f);
const data=JSON.parse(fs.readFileSync(path.join(root,'data/inventory.json'),'utf8'));
